...
#import pymysql # pip install pymysql (use for mysql database fatching)
import sqlite3
import os
class registration:
    def __init__(self,root):
        ...
        #-----------------bg image--------------------------
        self.bg=ImageTk.PhotoImage(file="Images/background.jpeg")
        bg=Label(self.root,image=self.bg).place(x=80,y=0,relwidth=1,relheight=1)
       
       
        #-----------------left image--------------------------
        self.left=ImageTk.PhotoImage(file="Images/side.jpeg")
        left=Label(self.root,image=self.left).place(x=92,y=100,width=450,height=530)
        #--------------Register Fream------------------
        frame1=Frame(self.root,bg="white")
        frame1.place(x=540,y=100,width=850,height=530)
        ...
        self.btn_img=ImageTk.PhotoImage(file="Images/register.jpeg")# register
        btn_register=Button(frame1,image=self.btn_img,bd=0,cursor="hand2",command=self.register_data).place(x=50,y=420,width=500,height=80)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or  self.txt_password.get()==""  or  self.txt_cpassword.get()=="":
            ...
            try:
                ...
                con.commit()   #(databse e jo jo changes hoyega as it save ho jayega)                
                con.close()                    
